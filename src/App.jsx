...
import { 
  BookOpen, Grid, Settings, ChevronRight, RotateCcw, 
  Volume2, Globe, User, Edit3, X, Zap, Trophy,
  Sparkles, Heart, Layers, Gamepad2, ArrowRight, CheckCircle, AlertCircle, HelpCircle, Star as StarIcon
} from 'lucide-react';
...
const AVATARS = [
  { id: 'cat', icon: '🐱', bg: 'bg-orange-100' },
  { id: 'shiba', icon: '🐕', bg: 'bg-yellow-100' },
  { id: 'rabbit', icon: '🐰', bg: 'bg-pink-100' },
  { id: 'bear', icon: '🐻', bg: 'bg-brown-100' },
  { id: 'onigiri', icon: '🍙', bg: 'bg-green-100' },
  { id: 'sakura', icon: '🌸', bg: 'bg-red-50' },
  { id: 'ramen', icon: '🍜', bg: 'bg-orange-50' },
  { id: 'dango', icon: '🍡', bg: 'bg-purple-50' },
];
...
const GlassCard = ({ children, className = "", onClick, noPadding = false, active = false, shine = false }) => (
  <div 
    ...
    className={`
      ...
      ${active 
        ? 'bg-white/80 dark:bg-gray-800/80 shadow-[0_8px_32px_0_rgba(31,38,135,0.15)] scale-[1.02]' 
        : 'bg-white/60 dark:bg-gray-900/40 hover:bg-white/70'
      }
      border-t border-l border-r border-b 
      border-t-white/80 border-l-white/60 border-r-white/40 border-b-white/20
      dark:border-t-white/20 dark:border-b-white/5
    ...
    `}
  >
    <div className="absolute inset-x-0 top-0 h-1/3 bg-gradient-to-b from-white/30 to-transparent pointer-events-none opacity-60"></div>
    {shine && (
      <div className="absolute inset-0 z-0 bg-gradient-to-tr from-transparent via-white/20 to-transparent translate-x-[-100%] animate-shimmer pointer-events-none"></div>
    )}
...
const NavItem = ({ icon: Icon, label, active, onClick }) => (
  <button 
    onClick={onClick}
    className={`
      relative flex flex-col items-center justify-center h-full flex-1 group
    `}
  >
    <div className={`
      relative p-3.5 rounded-2xl transition-all duration-500 cubic-bezier(0.34, 1.56, 0.64, 1)
      ${active 
        ? 'bg-gradient-to-tr from-blue-500 to-indigo-500 text-white shadow-lg shadow-blue-500/30 -translate-y-4 scale-110' 
        : 'text-gray-400 dark:text-gray-500 hover:bg-white/40'
      }
    `}>
      <Icon size={24} strokeWidth={active ? 2.5 : 2} />
    </div>
    <span className={`
      text-[10px] font-bold mt-1 transition-all duration-300 absolute bottom-0.5
...
const Avatar = ({ id, size = 'md', className = "" }) => {
  ...
  return (
    <div className={`
      rounded-full flex items-center justify-center shadow-[inset_0_2px_4px_rgba(255,255,255,0.5),0_4px_8px_rgba(0,0,0,0.1)]
...
const SectionHeader = ({ title, subtitle }) => (
    <div className="mb-6 px-4">
        <h2 className="text-3xl font-black text-gray-800 dark:text-white tracking-tight flex items-center drop-shadow-sm">
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 animate-gradient-slow">{title}</span>
          <Sparkles size={20} className="ml-2 text-yellow-400 fill-current animate-pulse" />
        </h2>
        {subtitle && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 font-bold opacity-80">{subtitle}</p>}
    </div>
)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="w-full max-w-md bg-white/90 dark:bg-gray-900/90 backdrop-blur-2xl rounded-[2.5rem] p-6 relative z-10 shadow-2xl animate-scale-up border border-white/50 ring-1 ring-white/50">
        ...
        <div className="text-center mb-6">
           ...
           <p className="text-gray-400 text-sm">{t.drawDesc}</p>
        </div>
        ...
        <div className="relative w-full aspect-square bg-white dark:bg-gray-800/50 rounded-3xl border-2 border-dashed border-gray-200 dark:border-gray-700 overflow-hidden touch-none mb-6 shadow-inner">
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none opacity-20">
            <span className="text-[200px] font-serif text-gray-400">{char}</span>
          </div>
          <canvas
            ref={canvasRef}
        ...
        <div className="flex gap-4">
          <button onClick={clearCanvas} className="flex-1 py-4 rounded-2xl bg-gray-100 dark:bg-gray-800 font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-200 transition-colors flex items-center justify-center">
...
const KanaView = ({ t, openCanvas }) => {
  ...
  return (
    <div className="space-y-6 pb-32 animate-fade-in">
      ...
      <div className="bg-white/40 dark:bg-gray-800/40 p-1.5 rounded-full flex backdrop-blur-xl mx-auto max-w-xs shadow-[inset_0_2px_4px_rgba(0,0,0,0.05)] ring-1 ring-white/40">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-2">
        {KANA_DATA.map((item, index) => {
          const char = mode === 'h' ? item.h : item.k;
          return (
            <GlassCard key={index} onClick={() => { speak(char); openCanvas(char); }} className="aspect-square flex flex-col items-center justify-center group !p-2 !rounded-3xl hover:-translate-y-1">
              <span className="text-3xl font-medium text-gray-800 dark:text-white mb-1 group-hover:scale-110 transition-transform duration-300 group-hover:text-blue-600">
                {char}
              </span>
              <span className="text-xs font-bold text-gray-400/80 uppercase tracking-widest">{item.r}</span>
            </GlassCard>
          )
        })}
      </div>
...
// --- 每日测验 (Quiz Mode) - UPDATED WITH VICTORY SCREEN ---
const QuizView = ({ t, isZh, vocabList, addXp, onFinish }) => {
  ...
  const scoreRef = useRef(0); // Use ref to track score synchronously
  ...
  const handleSelect = (option) => {
    ...
    setTimeout(() => {
      if (currentIndex < questions.length - 1) {
        ...
        // Show Victory Screen instead of instant exit
        setIsCompleted(true);
      }
    }, 1500);
  };
  ...
  // Victory Screen
  if (isCompleted) {
    const isPerfect = scoreRef.current >= 100; // Assuming 5 questions max score around 100+
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full animate-scale-up px-4">
        <div className="relative mb-8">
            ...
            <GlassCard className="!p-8 flex flex-col items-center !bg-white/80" shine={isPerfect}>
                <Trophy size={64} className={`mb-4 ${isPerfect ? 'text-yellow-500 animate-bounce' : 'text-blue-500'}`} fill="currentColor" />
                <h2 className="text-2xl font-black text-gray-800 mb-2">{isPerfect ? t.quizPerfect : t.quizFinish}</h2>
                <div className="text-gray-500 font-bold mb-6">{isPerfect ? t.quizGood : t.quizKeepGoing}</div>
                ...
                <div className="flex space-x-2 mb-6">
                    {[1, 2, 3].map(i => (
                        <StarIcon 
                            key={i} 
                            size={32} 
                            className={`${i <= stars ? 'text-yellow-400 fill-current' : 'text-gray-200'} transition-all duration-500`}
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in pb-20 pt-4 px-2">
       <div className="flex justify-between items-center mb-6">
          <button onClick={onFinish} className="p-2 bg-white/40 rounded-full hover:bg-white"><X size={24} /></button>
          <div className="flex flex-col items-center">
             <h3 className="font-bold text-gray-600">{t.quizTitle}</h3>
             <div className="flex space-x-1 mt-1">
                {questions.map((_, i) => (
                   <div key={i} className={`h-1.5 w-6 rounded-full transition-all ${i === currentIndex ? 'bg-blue-500' : i < currentIndex ? 'bg-green-400' : 'bg-gray-200'}`}></div>
                ))}
             </div>
          </div>
          <div className="font-black text-blue-600">{score}</div>
      </div>

      <div className="flex-1 flex flex-col justify-center items-center">
         {combo > 1 && <div className="text-yellow-500 font-black text-xl animate-bounce mb-4 drop-shadow-sm">🔥 {combo} Combo!</div>}
         
         <GlassCard className="w-full mb-8 flex flex-col items-center justify-center py-12 !bg-white/80">
             <h2 className="text-6xl font-medium text-gray-800 mb-2">{currentQ.answer.ja}</h2>
             <p className="text-gray-400">{currentQ.answer.ro}</p>
         </GlassCard>

         <div className="grid grid-cols-1 gap-3 w-full">
            {currentQ.options.map((opt, i) => {
              let stateClass = "bg-white/50 hover:bg-white/80 border-transparent";
              if (selectedOption) {
                 if (opt.id === currentQ.answer.id) stateClass = "bg-green-100 border-green-400 text-green-700 shadow-green-200";
                 else if (opt.id === selectedOption.id) stateClass = "bg-red-100 border-red-400 text-red-700 shadow-red-200 animate-shake";
  ...
  return (
    <div className="flex flex-col items-center h-[85vh] pb-20 animate-fade-in relative px-2">
      <div className="w-full flex justify-between items-center mb-6">
          <button onClick={onFinish} className="p-2 bg-white/40 backdrop-blur-md rounded-full hover:bg-white transition-colors">
              <ChevronRight size={24} className="rotate-180 text-gray-600" />
          </button>
          <div className="text-sm font-bold bg-white/30 backdrop-blur-md px-3 py-1 rounded-full text-gray-600 shadow-sm border border-white/20">
      ...
      <div className="relative w-full max-w-sm flex-1 max-h-[500px] perspective-1000 group z-10">
        ...
        >
          {/* Front */}
          <GlassCard className="absolute inset-0 backface-hidden flex flex-col !rounded-[2.5rem] !bg-white/80" shine={true}>
             <div className="flex justify-between items-start mb-4" onClick={e => e.stopPropagation()}>
                 <span className="px-3 py-1 bg-blue-100/50 text-blue-600 rounded-full text-xs font-bold uppercase tracking-wider">JP</span>
                 <div className="flex space-x-2">
                    <button onClick={() => speak(currentCard.ja)} className="p-2.5 bg-white/50 rounded-full text-blue-500 hover:scale-110 transition-transform shadow-sm">
                      <Volume2 size={20} />
                    </button>
                    <button onClick={() => toggleFavorite(currentCard.id)} className={`p-2.5 rounded-full transition-all hover:scale-110 shadow-sm ${isFav ? 'bg-pink-100 text-pink-500' : 'bg-white/50 text-gray-400'}`}>
             ...
             <div className="flex-1 flex flex-col items-center justify-center space-y-4">
                 ...
                 <p className="text-xl text-gray-400 font-medium tracking-wide">{currentCard.ro}</p>
             </div>
          ...
          {/* Back */}
          <GlassCard className="absolute inset-0 backface-hidden rotate-y-180 flex flex-col items-center justify-center !rounded-[2.5rem] !bg-gradient-to-br from-blue-50/90 to-purple-50/90 border border-purple-100">
            <span className="text-xs font-black text-purple-400 bg-white/60 px-4 py-1.5 rounded-full mb-8 uppercase tracking-widest shadow-sm">
                 {t.meaning}
             </span>
            <h2 className="text-4xl font-bold text-gray-800 dark:text-gray-900 text-center px-4 leading-relaxed">
      ...
      <button onClick={handleNext} className="mt-8 w-full max-w-xs bg-gray-900 dark:bg-white text-white dark:text-black py-4 rounded-2xl font-bold text-lg shadow-xl shadow-gray-900/20 active:scale-95 transition-all flex items-center justify-center backdrop-blur-sm group overflow-hidden relative">
    ...
    return (
        <div className="h-full flex flex-col animate-fade-in pb-20">
             <div className="flex justify-between items-center mb-6 px-2">
                <button onClick={onFinish} className="p-2 bg-white/40 rounded-full hover:bg-white transition-colors"><X size={24} /></button>
                <h3 className="font-bold text-xl text-gray-700">{t.matchTitle}</h3>
                <div className="w-10"></div>
            </div>
                    ...
                    return (
                        <div key={card.id} onClick={() => handleCardClick(card)}
                             className={`
                                ...
                                ${isSel ? 'bg-blue-100/90 border-blue-400 text-blue-600 scale-105 shadow-md' : 'bg-white/60 border-white/40 hover:bg-white/80 text-gray-700'}
                                ${isSel && isChecking && selected.length === 2 && selected[0].vocabId !== selected[1].vocabId ? 'bg-red-100 border-red-400 animate-shake' : ''}
                                backdrop-blur-md
                             `}>
...
const ProfileView = ({ t, isZh, toggleLang, user, updateUser, resetData }) => {
  const { level, progress, nextXp } = getLevelInfo(user.xp);
  const [isEditingAvatar, setIsEditingAvatar] = useState(false);
  const displayName = user.name || t.defaultName;

  return (
  <div className="space-y-6 pb-32 animate-fade-in max-w-md mx-auto">
    <SectionHeader title={t.tabProfile} />

    <div className="relative pt-6 px-4">
        <GlassCard className="flex flex-col items-center pt-12 pb-8 !rounded-[2.5rem] relative overflow-visible mt-6 !bg-white/70" shine={true}>
            <div className="absolute -top-12 cursor-pointer group z-20" onClick={() => setIsEditingAvatar(!isEditingAvatar)}>
                <Avatar id={user.avatarId} size="xl" className="shadow-[0_8px_20px_rgba(0,0,0,0.15)] ring-4 ring-white dark:ring-gray-800 group-hover:scale-110 transition-transform duration-300" />
                <div className="absolute bottom-0 right-0 bg-blue-500 text-white p-1.5 rounded-full shadow-md border-2 border-white">
                    <Edit3 size={14} />
                </div>
            </div>
            
            <div className="mt-8 mb-1 z-10 text-center">
            ...
            <div className="flex items-center space-x-2 mb-6 z-10">
                ...
                <span className="text-gray-500 text-sm font-bold">{Math.floor(user.xp)} / {nextXp} XP</span>
            </div>

            {isEditingAvatar && (
                <div className="mb-6 p-4 bg-white/60 backdrop-blur-xl rounded-2xl w-full animate-fade-in grid grid-cols-4 gap-2 border border-white/60 z-20 shadow-lg">
                    {AVATARS.map(a => (
                        <button key={a.id} onClick={() => { updateUser({...user, avatarId: a.id}); setIsEditingAvatar(false); }}
                            className={`p-2 rounded-xl flex justify-center hover:bg-white transition-colors ${user.avatarId === a.id ? 'bg-white shadow-md ring-2 ring-blue-400' : ''}`}>
            ...
            <div className="flex w-full px-2 space-x-3 z-10">
                <div className="flex-1 bg-orange-50/60 border border-orange-100 p-3 rounded-3xl flex flex-col items-center">
                    <Zap size={22} className="text-orange-500 mb-1 filter drop-shadow-sm" fill="currentColor" />
                    <span className="text-xl font-black text-gray-800">{user.streak}</span>
                    <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{t.days}</span>
                </div>
                <div className="flex-1 bg-purple-50/60 border border-purple-100 p-3 rounded-3xl flex flex-col items-center">
                    <Trophy size={22} className="text-purple-500 mb-1 filter drop-shadow-sm" fill="currentColor" />
                    <span className="text-xl font-black text-gray-800">{user.xp}</span>
                    <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{t.xp}</span>
                </div>
            </div>
            
            <div className="w-full mt-6 px-6 z-10">
    ...
    <div className="space-y-4 px-4">
        <button onClick={toggleLang} className="w-full flex items-center justify-between p-4 bg-white/50 dark:bg-gray-800/40 rounded-3xl hover:bg-white/70 transition-all shadow-sm hover:shadow-md border border-white/40 backdrop-blur-md group active:scale-[0.98]">
            <div className="flex items-center space-x-3">
                <div className="p-2.5 bg-indigo-100/80 text-indigo-600 rounded-2xl group-hover:scale-110 transition-transform"><Globe size={20} /></div>
                <span className="font-bold text-gray-700">{t.switchLang}</span>
            </div>
            <span className="text-sm font-bold text-gray-500 bg-white/60 px-3 py-1 rounded-xl shadow-sm">{isZh ? '中文' : 'EN'}</span>
        </button>
        <button onClick={() => { if(window.confirm(t.resetConfirm)) resetData(); }} className="w-full flex items-center justify-between p-4 bg-white/50 dark:bg-gray-800/40 rounded-3xl hover:bg-red-50/80 transition-all shadow-sm hover:shadow-md border border-white/40 backdrop-blur-md group active:scale-[0.98]">
            <div className="flex items-center space-x-3">
                <div className="p-2.5 bg-red-100/80 text-red-500 rounded-2xl group-hover:bg-red-200 transition-colors"><RotateCcw size={20} /></div>
                <span className="font-bold text-red-500">{t.resetData}</span>
            </div>
        </button>
    </div>
  </div>
...
const Onboarding = ({ t, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6 animate-fade-in">
      ...
      <div className="relative max-w-sm w-full bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-[2.5rem] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.1)] p-8 border border-white/60 text-center ring-1 ring-white/50">
        <div className="mb-6">
            <h1 className="text-3xl font-black text-gray-800 dark:text-white mb-2 tracking-tight">{t.onboardingTitle}</h1>
            <p className="text-gray-500 font-bold opacity-70">{t.onboardingDesc}</p>
        </div>
            ...
            {AVATARS.map(avatar => (
                <button 
                    ...
                    className={`
                        ...
                        ${selectedAvatar === avatar.id ? 'bg-white ring-4 ring-blue-200 scale-110 shadow-xl z-10' : 'bg-gray-50 hover:bg-white hover:shadow-md scale-100 opacity-70 hover:opacity-100'}
                    `}
                >
                    {avatar.icon}
                </button>
        ...
        <div className="bg-gray-50/50 dark:bg-gray-800 rounded-2xl p-2 mb-6 border border-gray-200 focus-within:border-blue-400 focus-within:bg-white focus-within:shadow-lg transition-all">
            <input 
                type="text" 
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder={t.namePlaceholder}
                className="w-full bg-transparent text-center text-xl font-bold p-2 outline-none text-gray-800 dark:text-white placeholder:text-gray-300"
                maxLength={10}
            />
        </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen relative font-sans text-gray-900 bg-[#eff4ff] dark:bg-[#0f172a] overflow-hidden selection:bg-blue-200 selection:text-blue-900">
      ...
      {/* 极光背景动画 - 优化颜色流转 */}
      <div className="fixed inset-0 -z-10 bg-[radial-gradient(circle_at_50%_10%,_#dbeafe_0%,_transparent_50%),radial-gradient(circle_at_90%_60%,_#fce7f3_0%,_transparent_40%),radial-gradient(circle_at_10%_60%,_#ede9fe_0%,_transparent_40%)] opacity-80 animate-pulse-slow"></div>
      
      {/* 动态噪点层 - 增加细微的纸质纹理感 */}
      <div className="fixed inset-0 -z-10 opacity-[0.04] pointer-events-none" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")` }}></div>

      <main className={`max-w-md mx-auto h-[100dvh] overflow-y-auto no-scrollbar ${practiceMode ? 'p-4' : 'px-4 pt-10 pb-32'}`}>
        {user !== 'NEW' && !practiceMode && (
          <div className="min-h-full">
                ...
                <div className="animate-fade-in space-y-4 pb-24">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                        <div onClick={() => { setPracticeMode('flashcards'); setFilterFavs(false); }} 
                             className="bg-white/60 backdrop-blur-xl p-5 rounded-[2rem] shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all cursor-pointer group border border-white/60 relative overflow-hidden flex flex-col justify-between h-48">
                            <div className="absolute -right-4 -top-4 opacity-5 text-8xl group-hover:scale-110 transition-transform">🎴</div>
                            <div className="w-12 h-12 bg-blue-100/80 rounded-2xl flex items-center justify-center text-blue-500 mb-2 group-hover:rotate-12 transition-transform shadow-sm"><Layers size={22} /></div>
                            <div>
                                <h3 className="text-lg font-black text-gray-800 leading-tight">{t.modeFlashcards}</h3>
                                <p className="text-gray-400 text-xs mt-1 font-bold">{t.modeFlashcardsDesc}</p>
                            </div>
                        </div>
                        ...
                        <div onClick={() => { setPracticeMode('matching'); }} 
                             className="bg-white/60 backdrop-blur-xl p-5 rounded-[2rem] shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all cursor-pointer group border border-white/60 relative overflow-hidden flex flex-col justify-between h-48">
                            <div className="absolute -right-4 -top-4 opacity-5 text-8xl group-hover:scale-110 transition-transform">🧩</div>
                            <div className="w-12 h-12 bg-purple-100/80 rounded-2xl flex items-center justify-center text-purple-500 mb-2 group-hover:rotate-12 transition-transform shadow-sm"><Gamepad2 size={22} /></div>
                            <div>
                                <h3 className="text-lg font-black text-gray-800 leading-tight">{t.modeMatching}</h3>
                                <p className="text-gray-400 text-xs mt-1 font-bold">{t.modeMatchingDesc}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div onClick={() => { setPracticeMode('quiz'); }} 
                         className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 rounded-[2rem] shadow-lg shadow-indigo-500/20 hover:shadow-indigo-500/30 hover:scale-[1.01] transition-all cursor-pointer group relative overflow-hidden">
                        <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                        <div className="flex items-center justify-between relative z-10">
                            <div className="flex items-center space-x-4">
                                <div className="p-3 bg-white/20 rounded-full text-white backdrop-blur-sm"><CheckCircle fill="currentColor" size={24} /></div>
                                <div className="text-white">
                                    <h3 className="font-black text-xl">{t.modeQuiz}</h3>
                    ...
                    {(user.favorites?.length > 0) && (
                         <div onClick={() => { setPracticeMode('flashcards'); setFilterFavs(true); }} 
                             className="bg-pink-50/60 backdrop-blur-xl p-6 rounded-[2rem] border border-pink-100 cursor-pointer flex items-center justify-between hover:bg-pink-100/80 transition-colors mt-4">
                            <div className="flex items-center space-x-4">
                                <div className="p-3 bg-pink-200 text-pink-600 rounded-full"><Heart fill="currentColor" size={20} /></div>
                                <div>
                                    <h3 className="font-bold text-pink-900">{t.onlyFavorites}</h3>
                                    <p className="text-xs text-pink-600 font-bold">{user.favorites.length} words</p>
                                </div>
                            </div>
            ...
            {activeTab === 'profile' && <ProfileView t={t} isZh={isZh} toggleLang={() => {setLang(l=>l==='zh'?'en':'zh'); localStorage.setItem('kawaii_lang', l==='zh'?'en':'zh')}} user={user} updateUser={saveUser} resetData={resetData} />}
          </div>
        )}
      ...
      {!practiceMode && user !== 'NEW' && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-full max-w-[280px] sm:max-w-xs animate-slide-up">
          <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-2xl border border-white/40 shadow-[0_20px_40px_-10px_rgba(0,0,0,0.1)] rounded-[2.5rem] p-2 flex items-center justify-between px-6 ring-1 ring-white/50">
